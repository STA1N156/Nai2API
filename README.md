...
- 官方密钥：前台及 `/generate?token=pst-...` 支持用户自己的 NovelAI 密钥，仅使用该账号、不扣站内额度；前台显示 Anlas、V5 剩余额度和会员类型。同一官方密钥的任务依次生成，不会切换到公共账号池。
- 额度扣费：V4.5 普通图扣 `1` 点，V5 普通图扣 `8` 点，2K 扣 `15` 点，4K 扣 `25` 点，失败会自动退回
- NovelAI 账号池：V5 优先使用仍有免费额度的账号，免费账号忙满或耗尽后优先使用点数更多的账号
- 并发控制：每个 NovelAI 账号同时生成 `1` 张，多账号可并行分担请求
- 排队机制：网页生图可排队并显示状态；图片 URL 请求会等待，超过 60 秒返回“服务器繁忙”
- 图片缓存：同参数可命中缓存，避免重复扣费；后台可按分辨率及 V4.5/V5 版本筛选、预览、搜索和清理
- 生成尺寸：仅保留横图、竖图、方图三种常用规格
- 步数策略：仅网页生成支持 `1–50` 步；原任务接口 `/api/jobs`、`/generate` 图片 URL 仍最高 `28` 步，OpenAI 接口固定 `28` 步
- 高级设置：采样器、提示词引导强度、引导重缩放默认折叠；迭代步数、提示词引导强度、引导重缩放支持滑条调节；画师串与负面提示词可单独展开。
...
官方密钥在前台仅保存于当前浏览器会话，服务端不将原始密钥写入数据库。带 `token` 的 URL 属于敏感链接，不要公开分享或放入公开网页；反向代理访问日志也应隐藏该参数。官方额度由 NovelAI 决定，页面查询时读取官方数据，不在本地估算扣除。
...
每个 NovelAI 账号都有自己的并发计数。V5 标准图优先选择仍有免费额度且空闲的账号；免费账号全部忙满或耗尽后，再按剩余 NovelAI 点数从高到低选择。V4.5 标准图仍按当前负载和最近使用时间轮换，2K/4K 会检查账号点数是否足够。
...
- 导入/导出账号 token
- 查看成功/失败统计
- 最近 1 分钟出图按完成时间统计；出图量和每日请求数排除缓存命中，点数消耗仍保留实际扣费
- 手动刷新、进入后台时等待最新统计；普通自动刷新继续使用 30 秒统计缓存，过期后后台更新
- 重置监控数据
- 每 5 分钟自动刷新点数、会员和 V5 免费额度（单次查询超时 15 秒）
...
网页使用 `POST /api/web/jobs`，任务查询、排队与预览仍复用 `/api/jobs` 的现有接口。服务端按实际步数计算价格，忽略客户端提交的 `cost`；多张图片逐张计费，失败退回该任务已扣的点数。

| 步数 | V4.5 标准图 | V5 标准图 | 2K（两种模型） | 4K（两种模型） |
| --- | --- | --- | --- | --- |
| 1–28 | 1 | 8 | 15 | 25 |
| 29–35 | 6 | 12 | 19 | 29 |
| 36–45 | 8 | 14 | 21 | 31 |
...
所有画幅使用相同档位规则。上述是用户密钥点数，不是 NovelAI Anlas。NovelAI 账号点数只以官方额度刷新结果为准，本地不预估或扣减；自动刷新会同步真实余额。超过 28 步和 2K/4K 请求优先分配给有点数且余额较多的账号。上游返回点数不足时，暂停该账号的付费请求分配，等额度刷新后重新判断。

URL 嵌入代码仍最高 28 步，原 API 与 OpenAI 接口价格不变。
...
| `tag` | 用户提示词 |
| `model` | `nai-diffusion-4-5-full`（默认，1 点）或 `nai-diffusion-5-full`（8 点） |
| `artist` | 质量前缀或画师前缀 |
| `size` | `竖图`、`横图`、`方图` |
...
OpenAI 兼容接口可用以下两档高分辨率（固定 28 步，计费不随网页步数设置变化）：

| 模型 | 前缀 | 扣费 | 竖图 | 横图 | 方图 |
| --- | --- | ---: | --- | --- | --- |
| V4.5 Full | 无前缀 | `1` 点 | `832 x 1216` | `1216 x 832` | `1024 x 1024` |
| V5 Full | 无前缀 | `8` 点 | `832 x 1216` | `1216 x 832` | `1024 x 1024` |
...
`GET /api/jobs/{任务ID}?token={生图密钥}` 的路径、鉴权和原有返回字段保持不变。

- `queuePosition / queuedCount` 表示递增的排队进展，例如 `1/300 → 2/300`，不是当前排位或生成百分比。分母在入队时确定，后来加入的任务不会把它变大。
- 数字按真实队列更新，不播放模拟进度动画；等待账号时不会提前显示 `300/300`，真正进入 `running` 才到满值。无需排长队的 `1/1` 仍须检查 `status`。
- `generationProgress.percent` 是上游生成进度。即使达到 100%，也只在 `status === 'done'` 时读取 `imageUrl`；`failed` 才代表任务失败。
- 同一任务建议串行轮询，上一条请求结束后再发下一条，避免较旧的响应覆盖较新的状态。

...
如果你要完整备份图片缓存，需要直接备份 `/data/library.json`，或者在服务器层面对 `/data` Volume 做备份。
...
这一步很重要。Nai2API 的数据库文件在 `/data/library.json`，不挂载 Volume 的话，服务重启或重新部署后数据可能回到初始状态。
...
1. 停止本地服务，确保 `data/library.json` 不再写入。
2. 备份本地 `data/library.json`。
3. 通过 Zeabur 文件管理或其他服务器文件方式上传到 `/data/library.json`。
4. 重启服务。
